...
class EnvRunner(Runner):
    ...
    def run(self):
        ...
        font = FontProperties(fname="C:/Windows/Fonts/simsun.ttc")  # 设置标签字体
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                if episode % 10 != 0 and episode % 5 == 0:
                    ...
                    x0.append(obs[0][0][0])
                    y0.append(obs[0][0][1])
                    plt.plot(x0, y0, color="blue", linewidth=1.0, linestyle="-")
            ...
            if episode % self.log_interval == 0:
                ...
                reward.append(np.mean(self.buffer.rewards) * self.episode_length)
        ...
        plt.rcParams['xtick.direction'] = 'in'  # 将x周的刻度线方向设置向内
        plt.rcParams['ytick.direction'] = 'in'  # 将y轴的刻度方向设置向内
        plt.plot(episodes_num, reward, color="blue", linewidth=1.0, linestyle="-")
        plt.xlabel('训练轮次', fontproperties=font, size=16)
        plt.ylabel('奖励', fontproperties=font, size=16)
        plt.savefig(r'.\picture\reward.png')
        plt.cla()
